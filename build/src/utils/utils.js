...
export const postData = async (data, headers, next) => {
    try {
...
export const DataWithRetry = async ({ url, next, retry = 3, delay = 1000, responseType = 'json', headers, data }) => {
    try {
...
export const postBtn = async ({ viewState, jsessionid, next, pageIndex }) => {
...
export const parseXml = async ({ xml, jsessionid, next, index }) => {
    const id = index === 0 ? 'listarDetalleInfraccionRAAForm:pgLista' : 'listarDetalleInfraccionRAAForm:dt';
...
async function downloadPdf({ jsessionid, viewState, paramUuid, botonId, outputPath, next }) {
...
const getParams = ({ type, pageIndex, viewState }) => {
    const params = {
        "postBtn": {
    ...
    return new URLSearchParams(params[type]);
};
